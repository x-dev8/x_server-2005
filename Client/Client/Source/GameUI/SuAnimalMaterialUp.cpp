...
bool CUI_ID_FRAME_SuAnimalMaterialUp::_LoadUI()
{
	...
	if ( dwResult == 0 )
	{
		MESSAGE_BOX("读取文件[Data\\UI\\SuAnimalMaterialUp.MEUI]失败")
		return false;
	}
...
void CUI_ID_FRAME_SuAnimalMaterialUp::PetSelectChange(int nindex)
{
	if ( !m_pID_FRAME_PetMaterialUp )
		return ;
	m_pID_TEXT_NowApperception->SetText("");//当前悟性
	RefreshMoney();
	if (nindex <0)  return ;
...
void CUI_ID_FRAME_SuAnimalMaterialUp::RefreshMonsy(SPetItem &mount)
{
	...
	int Rate = thePetSettingConfig.GetSavvyUpItemRate(quality,m_level,mount.baseInfo.savvy); //成功率
	int money = thePetSettingConfig.GetSavvyUpItemMoney(quality,m_level,mount.baseInfo.savvy);//所需金钱
...
// 					&&pRestore->specialFunction == ItemDefine::EPetFunc_AddSavvy )
// 				{
// 					if (mount.baseInfo.savvy < pRestore->petSavvy && (pRestore->petSavvy -mount.baseInfo.savvy)<=3 ) //存在可以提升宠物悟性的道具
// 					{
