...
float4 FocalPlane = float4( 0.0f, 0.0f, 0.2f, -0.1f );
                 ...
                 out float4 oCol : COLOR0 )
{
    ...
    float3 ColorPos = tex2D( g_samSrcPosition, Tex );
    
    //oCol = float4( ColorPos.rgb, 1.f);  

    //float Blur = dot( tex2D( g_samSrcPosition, Tex ), FocalPlane );
    float Blur = abs( ColorPos.r - FocalPlane.w );
    
    //oCol = float4(ColorPos, 1.f);    
...
<
    ...
    float4 Parameter0Def = float4( 0.0f, 0.0f, 0.2f, -0.1f );
