...
public:	
    ...
    bool CanPushItem( SCharItem &item, bool& bUseNewGrid );
	...
	int PopItemByGuID(__int64 GuID,unsigned short ustCount, funPlayerLog pLogFunc, BasePlayer* pPlayer, std::string strLog);
	...
	bool GetBagEnablesByIndex(short Index){return m_bEnables[Index];}
public:
    //相关背包指针
...
private:
    ...
    //是否能放入(非独占类)物品
    bool CanPushNonExclusiveItem( unsigned short ustItemID, unsigned short ustCount, bool bBind, bool& bUseNewGrid );
    //是否能取出(非独占类)物品
