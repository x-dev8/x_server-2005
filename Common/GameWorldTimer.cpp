...
#include <ctime>
#include <windows.h>
#include "../server/game_server/Server.h"
//#include "MsgDef.h"
#include "Script/ScriptMgr.h"
#include "helper.h"
#include "../server/game_server/cfgData.h"
...
void CGameWorldTimer::UpdateTimeToClient( C_Character* pChar )
{
	MsgUpdateTime msg;
	HelperFunc::SafeNCpy( msg.szYearName,
		m_strGameYear.c_str(),
		MsgUpdateTime::YEAR_NAME_LENGTH );
...
void CGameWorldTimer::Run()
{
	...
	if ( m_nGameMinute == 0 )
	{
		...
		HelperFunc::SafeNCpy( g_Cfg.m_szYearName, m_strGameYear.c_str(), 32 );
	...
	//// 天机元年一月一日子时一刻
	//char szBuf[128];
	//m_strGameTime = m_strGameYear;	// 天机
...
int CGameWorldTimer::GetRealTime( ETimeType type )
{
	...
	default:
		{
			GetErrorLog()->logString("Failed: CGameWorldTimer::GetRealTime()");
		}
		break;
	}
	return 0;
