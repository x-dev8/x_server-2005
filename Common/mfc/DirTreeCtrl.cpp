...
void CDirTreeCtrl::DisplayPath(HTREEITEM hParent, LPCTSTR strPath)
{
	...
	while ( bFind )
	{
		...
		{	
			...
			for( int n = 0; n < m_exts.size(); n++ )
			{
				CString& ext = m_exts[n];
				int l = ext.GetLength();
				if( nLength >= ext.GetLength() && 
					stricmp( &pszFilename[nLength-ext.GetLength()], ext  ) == 0 )
				{
	...
	if ( m_bFiles )
	{
		...
		for ( i = 0; i < strFileArray.GetSize(); i++ )
		{
			...
			int nFileID = m_vectorFile.size();
			SetItemData( hItem, nFileID+1 );
			m_vectorFile.push_back( strFileArray.GetAt(i));
		}
	}
