			...
			case PIT_BOOL:
				{
					...
					int nTop = rect.top+(rect.Height()-size)/2;
					CRect rc( nLeft, nTop, nLeft+size, nTop+size );
					dc.Rectangle( rc );
					//dc.Draw3dRect( &rc, RGB(128, 0, 0), RGB(0, 0, 0));
					//dc.FillSolidRect(&rc, RGB(255,255,255) );
					//dc.Rectangle( rc );
					CString	strBool;
					if( *(BOOL*)pItem->m_pValue )
					{
						//strBool = "■";
...
void CPropertyList::UpdateCallback()
{
	if( m_pUpdateCallbackFun )
		m_pUpdateCallbackFun();
}
void CPropertyList::OnSelchange() 
{
	...
	case PIT_BOOL:
		{
			...
			//BOOL b = *(BOOL*)pItem->m_pValue;
			//*(BOOL*)pItem->m_pValue = !b;
	...
	//CListBox::SendMessage( LBN_SELCHANGE );
	//if( GetParent() )
