		...
		*************************************************************************/
		...
		size_type encode(const utf32* src, utf8* dest, size_type dest_len, size_type src_len = 0) const
		{
			...
			for (UINT idx = 0; idx < src_len; ++idx)
			{
				...
				{
					//由于32数据会造成丢失数据错误,这里为了通过调试,故意失效,以后还要查明原因	(关键字:测试期)
					*dest++ = (utf8)(((cp >> 18) | 0xF0)				&0xFF);
					*dest++ = (utf8)((((cp >> 12) & 0x3F) | 0x80)		&0xFF);
					*dest++ = (utf8)((((cp >> 6) & 0x3F) | 0x80)		&0xFF);
					*dest++ = (utf8)(((cp & 0x3F) | 0x80)				&0xFF);
					destCapacity -= 4;
				}
