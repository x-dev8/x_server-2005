...
BOOL MexSkinMesh::ConvertToIndexedBlendedMesh( LPDIRECT3DDEVICE9 pDev, IMex* pMex )
{
	...
	for( int nChunk = 0; nChunk < bc->m_vectorChunk.size(); nChunk++ )
	{
		...
		D3DXVECTOR3* pVertices = (D3DXVECTOR3*)pGeomChunk->GetVertices();
		D3DXVECTOR3* pNormals = (D3DXVECTOR3*)pGeomChunk->GetNormals();
		D3DXVECTOR2* pUVs = (D3DXVECTOR2*)pGeomChunk->GetUVs( 0 );
