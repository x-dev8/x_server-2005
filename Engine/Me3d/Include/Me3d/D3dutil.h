...
//		 @return Boolean representing success or failure
//-----------------------------------------------------------------------------
BOOL D3DUtil_ConvertD3DXMeshToMesh( LPD3DXMESH& D3DMesh, CMexChunkBase& DestMesh,
									 INT NumUVs );
...
//-----------------------------------------------------------------------------
BOOL D3DUtil_GenerateUVs(CMexChunkBase& StaticMesh, UINT TexCoordIndex,
						 FLOAT &MaxDesiredStretch, UINT &ChartsGenerated);
...
// @return TRUE if successful
//-----------------------------------------------------------------------------
BOOL D3DUtil_GenerateLOD(CMexChunkBase& StaticMesh, INT DesiredLOD,
						 INT DesiredTriangles);
...
class CD3DArcBall
{
    ...
    D3DXMATRIX     m_matRotation;         // MeMatrix for arcball's orientation
    D3DXMATRIX     m_matRotationDelta;    // MeMatrix for arcball's orientation
    D3DXMATRIX     m_matTranslation;      // MeMatrix for arcball's position
    D3DXMATRIX     m_matTranslationDelta; // MeMatrix for arcball's position
...
public:
    ...
    VOID SetProjParams( FLOAT fFOV, FLOAT fAspect, FLOAT fNearPlane,
                        FLOAT fFarPlane );
	//   added 2003.08.15
	void SetOrthoLH( D3DXVECTOR3 vMin, D3DXVECTOR3 vMax );

	void SetLeftHand(bool bLeftHand)	{m_bLeftHand = bLeftHand;}
