...
static FileIO::CVFS g_SlkVFS;

static void (*g_pfnSlkLoadingCallBack)( const char* pszString ) = NULL;
static void OutputSlkLoadingMessage( char* pszString, ... )
{
...
static char g_szValueTemp[ 64000 ];
static char g_szStringValueTemp[257];

CMySlkReader::CMySlkReader()
{
	guardfunc
	m_iMaxLineNo = 0;
	m_pLastFindLine = 0;
...
void CMySlkReader::FreeLine(SLine*pLine)
{
	SField *pField;
...
char *CMySlkReader::GetFieldValue( char *szString,CMySlkReader::SField &field )
{
	//CSALocker locker(this);
	char *pcFind;
	int iSize;
...
// 				sprintf_s( field.data.szValue, nLength + 1, "%s", strContent.c_str() );
			...
			field.iFieldType = field_string;
			int nLength = strlen(g_szValueTemp);
			field.data.szValue = new char[ nLength + 1];
			memcpy(field.data.szValue, g_szValueTemp, nLength);
			field.data.szValue[nLength] = 0;
		}
...
char* CMySlkReader::GetRowOrLineNO(char *szString,int &iValue )
{
	//CSALocker locker(this);
	char *pcFind;
...
CMySlkReader::SLine *CMySlkReader::FindLine( int iLine,bool bAddIfNotFound )
{
	//CSALocker locker(this);
	/*listLine::iterator it;
	for( it = m_list.begin() ; it != m_list.end() ; it ++ )
...
int	 CMySlkReader::FindFirstLineByIntField( int iFieldNo,int iValue )
{
	//CSALocker locker(this);
...
int	 CMySlkReader::FindLineByIntField( int iFieldNo,int iValue )
{
	//CSALocker locker(this);
	listLine::iterator it;
...
void CMySlkReader::SetValue( CMySlkReader::SLine *pLine,int iX,SField &field  )
{
	//CSALocker locker(this);
	if( !pLine )
		return;
...
bool CMySlkReader::ReadFromFile( const char *szFile )
{
	//CSALocker locker(this);
...
bool CMySlkReader::ReadFromString( char *szString,int iSize )
{
	//CSALocker locker(this);
...
int		CMySlkReader::GetFieldNumOfLine()
{
	//CSALocker locker(this);
	if( !m_pCurGetLine )	
		return 0;
...
int CMySlkReader::GetCurrentLineIndex(void)
{
	if(m_pCurGetLine != NULL)
		return m_pCurGetLine->iLineNo;

	return -1;
}

CMySlkReader::SField *CMySlkReader::GetFieldOfLine( int iFieldNo )
{
	//CSALocker locker(this);
	if( !m_pCurGetLine )	
		return 0;
...
int CMySlkReader::GotoNextLine( int iLine )
{
	//CSALocker locker(this);
	if( iLine > m_iMaxLineNo )
		return ret_readover;
...
void CMySlkReader::OutDebugLine(SLine*pLine,FILE *fp)
{
	//CSALocker locker(this);
	SField *pField = NULL;
...
void CMySlkReader::OutDebugInfo()
{
...
bool CMySlkReader::GetIntField( int iCol,int &iValue )
{
	//CSALocker locker(this);
	//
	SField *pField;
	...
	switch( pField->iFieldType )
	{
	case CMySlkReader::field_int: 
		iValue = pField->data.iValue;
		break;
	case CMySlkReader::field_float: 
		return false;
		break;
	case CMySlkReader::field_string:
		return false;		
	}	
	return true;
}

bool CMySlkReader::GetFloatField( int iCol, float &fValue )
{
	//CSALocker locker(this);
	SField *pField;
	pField = GetFieldOfLine( iCol );
...
bool CMySlkReader::GetStringField( int iCol,string& strRetValue )
{
	//CSALocker locker(this);
	CMySlkReader::SField *pField;
	pField = GetFieldOfLine( iCol );
	if( !pField )
		return false;
	if( CMySlkReader::field_string != pField->iFieldType )
		return false;
...
bool CMySlkReader::GetStringField( int iCol,char **pstrRetValue,CFlyMemAllocer*pAllocer )
{
	//CSALocker locker(this);
	int iStringSize;
	CMySlkReader::SField *pField;
...
//////////////////////////////////////////////////////////////////////////
int CMySlkReader::ConvHeaderToId( const char* pszCol )
{
	//CSALocker locker(this);
	if( !pszCol )
	{
...
bool CMySlkReader::GetIntField( const char* pszCol,int &iValue )
{
	//CSALocker locker(this);
...
//读取string
bool CMySlkReader::GetStringField( const char* pszCol,std::string& pstrRetValue)
{
	//CSALocker locker(this);
	int nCol = ConvHeaderToId( pszCol );
	if( nCol == -1 )
...
//读取string
bool CMySlkReader::GetStringField( const char* pszCol,char **pstrRetValue,CFlyMemAllocer*pAllocer )
{
	//CSALocker locker(this);
	int nCol = ConvHeaderToId( pszCol );
	if( nCol == -1 )
...
//读取浮点
bool CMySlkReader::GetFloatField( const char* pszCol, float &fValue )
{
	//CSALocker locker(this);
	int nCol = ConvHeaderToId( pszCol );
	if( nCol == -1 )
...
void CMySlkReader::ClearBlankRow()
{
	//CSALocker locker(this);
...
void CMySlkReader::InitFiledName()
{
	//CSALocker locker(this);
    GotoNextLine( 2 );
...
int CMySlkReader::GetFiledNameID( const char * strName )
{
	//CSALocker locker(this);
...
bool CMySlkReader::GetIntFieldByName( const char* pszColName,int &iValue )
{
	//CSALocker locker(this);
	iValue = 0;
...
//bool CMySlkReader::GetIntFieldByName( const char* pszColName,short &shValue )
//{
//	int nValue = 0;
...
//}
//读取string
bool CMySlkReader::GetStringFieldByName( const char* pszColName,std::string& pstrRetValue)
{
	//CSALocker locker(this);
    int iCol = GetFiledNameID( pszColName );
    if( iCol == -1 )
...
//读取string
bool CMySlkReader::GetStringFieldByName( const char* pszColName,char **pstrRetValue,CFlyMemAllocer*pAllocer )
{
	//CSALocker locker(this);
    int iCol = GetFiledNameID( pszColName );
    if( iCol == -1 )
...
//读取浮点
bool CMySlkReader::GetFloatFieldByName( const char* pszColName, float &fValue )
{
	//CSALocker locker(this);
    int iCol = GetFiledNameID( pszColName );
    if( iCol == -1 )
...
//
//bool CMySlkReader::InsertLine( int nLineIndex )
//{ 
//    return 0;
//}
//
//bool CMySlkReader::DeteleLine( int nLineIndxe ) 
//{ 
//    return true;
//}
