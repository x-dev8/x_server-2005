...
bool CharChunkWR::LoadItemData( uint8* bybuff, DWORD dwsize, SCharItem* pItem )
{
    if (bybuff == 0||dwsize == 0||bybuff < 0||dwsize < 0)
        return false;

    int nSize = dwsize;
    CDataChunkLoader l( bybuff, nSize );

    pItem->Read( l.GetPointer().byte, l.GetBufferSize() );

    return true;
}

bool CharChunkWR::LoadMountData( uint8* bybuff, DWORD dwsize, SMountItem* pItem )
{
    if (bybuff == 0||dwsize == 0||bybuff < 0||dwsize < 0)
        return false;
