#include "StdAfx.h"
#include "Chatlistcontrol.h"
#include <assert.h>
#include "PropertyDlg.h"

CChatListControl::CChatListControl(void)
{
	m_nType = Type_ChatList;

	m_nWidth = 140;
	m_nHeight = 160;
	m_strID.Format( "ID_CHATLIST_%d", rand() );

	m_colComments = RGB(128,0,0);
	m_colSelBar = RGB(0,255,0);
	m_nItemHeight = 14;

	m_ProList[Item_List_Comments_State].SetItem( PIT_COMMENTS, "<ChatList>", NULL );
...
void CChatListControl::AddToPropertyDlg()
{
...
void CChatListControl::AddToUIObjectDlg()
{
	CBaseControl::AddToUIObjectDlg();
	m_ScrollBar.AddToUIObjectDlg();
}

bool CChatListControl::SetData( const S_BaseData* /*[In]*/pData )
{
	assert( pData );
	S_ChatListData *pListData = (S_ChatListData*)pData;
...
bool CChatListControl::GetData( S_BaseData* /*[Out]*/pData )
{
	assert( pData );
	S_ChatListData *pListData = (S_ChatListData*)pData;
...
void CChatListControl::Render( CDC* pDC, const RECT* pFatherRc )
{
...
bool CChatListControl::MouseMove( const POINT point, const RECT* pFatherRc )
{
	int nX,nY;
	GetPos( &nX, &nY, pFatherRc );
...
void CChatListControl::GetData_Event( int index )
{
