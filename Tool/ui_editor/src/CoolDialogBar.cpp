...
BOOL CCoolDialogBar::Create(CWnd* pParentWnd, CDialog *pDialog, char* szTitle, UINT nID, DWORD dwStyle) 
{
    ...
    CString wndclass = AfxRegisterWndClass(CS_DBLCLKS, LoadCursor(NULL, IDC_ARROW),
        m_brushBkgd, 0);
    if (!CWnd::Create(wndclass, szTitle, dwStyle, CRect(0,0,0,0),
        pParentWnd, 0))
        return FALSE;
...
//------- ÏÔÊ¾    (By WitSun at 2004-5-12)
void CCoolDialogBar::Show( CFrameWnd* pFmWnd, CDialog* pDlg, UINT nDlgId, char* pTileName, DWORD dwStyle )
{
	if(::IsWindow(m_hWnd))
	{
		pFmWnd->ShowControlBar(this, TRUE, FALSE);
		return;
	}

	if ( !Create(pFmWnd, pDlg, pTileName, nDlgId, dwStyle ) )
	{
