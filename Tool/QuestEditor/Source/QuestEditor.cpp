...
BEGIN_MESSAGE_MAP(CQuestEditorApp, CWinApp)
	ON_COMMAND(ID_APP_ABOUT, &CQuestEditorApp::OnAppAbout)
...
BOOL CQuestEditorApp::InitInstance()
{
	...
	pDocTemplate = new CSingleDocTemplate(
		IDR_MAINFRAME,
		RUNTIME_CLASS(CQuestEditorDoc),
		RUNTIME_CLASS(CMainFrame),       // main SDI frame window
		RUNTIME_CLASS(CQuestEditorView));
	if (!pDocTemplate)
		return FALSE;
...
void CQuestEditorApp::ShowVersionInformation()
{
	CString strVersion = "";

	strVersion.Format("ÈÎÎñ±à¼­Æ÷ %s", ApplicationConfig::Instance().GetFullVersion());

	theMainFrame->SetWindowText(strVersion);
}