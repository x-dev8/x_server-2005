...
bool GameCamera::Create()
{
	...
	swLuaScript* pLua = view->GetLua();
	// 读配置文件参数
	double lookup, lookdown, mindist, maxdist, defaultdist;
	if( pLua->GetGlobalNumber("CAMERA_LOOK_UP_LIMIT", &lookup) )
		CAMERA_LOOK_UP_LIMIT = -lookup;
	if( pLua->GetGlobalNumber("CAMERA_LOOK_DOWN_LIMIT", &lookdown) )
	{
		CAMERA_LOOK_DOWN_LIMIT = -lookdown;
		m_ptSphericalPos.z = CAMERA_LOOK_DOWN_LIMIT;
	}
	if( pLua->GetGlobalNumber("CAMERA_MIN_DISTANCE", &mindist) )
		CAMERA_MIN_DISTANCE = mindist;
	if( pLua->GetGlobalNumber("CAMERA_MAX_DISTANCE", &maxdist) )
		CAMERA_MAX_DISTANCE = maxdist;
	if( pLua->GetGlobalNumber("CAMERA_DEFAULT_DISTANCE", &defaultdist) )
		CAMERA_DEFAULT_DISTANCE = defaultdist;

	double speed;
	if( pLua->GetGlobalNumber("CHARACTOR_SPEED", &speed) && speed >= 0)
	{
	...
	double fRight;
	double fTop;
	bFindPath = (pLua->GetGlobalNumber("CAMERA_FRUSTUM_RIGHT", &fRight)
					&& pLua->GetGlobalNumber("CAMERA_FRUSTUM_TOP", &fTop));
...
void GameCamera::Update( float fTotalTime, float fDeltaTime )
{
    ...
    //CaluEyeAndLookat( vEye,vLookAt );    //&& 这里会根据vLookAt而调整vEye
	CaluEyeAndLookatEx( vEye,vLookAt );
...
{
	//&& 下面这段代码的意思，是根据vLookat和m_ptCameraRot，算出vEye，所以我们要改变摄象机位置的时候应该优先改变m_ptCameraRot。
	//&& 而且这个函数会每帧都会执行.
...
void GameCamera::CaluEyeAndLookatEx( D3DXVECTOR3 &vEye,D3DXVECTOR3 &vLookat )
{
	D3DXVECTOR3 vDist(0,0,1);
	D3DXVec3Normalize( &vDist, &vDist );
	...
	m_ptCameraPos.x = vLookat.x + vDist.x * m_iOffsetCamera;  // 我在游戏中得到的距离是15.123, , 
	m_ptCameraPos.y = vLookat.y + vDist.y * m_iOffsetCamera;  // 在构造函数中,m_iOffsetCamera=15, 如果使用滚轮控制则打开
	m_ptCameraPos.z = vLookat.z + vDist.z * m_iOffsetCamera;  // CToolGameCamera::OnMouseWheel( ) 注释.
