...
HRESULT CD3DApplication::Create( HINSTANCE hInstance )
{
    ...
    if( m_hMainWnd == NULL)
    {
        ...
        m_hMainWnd = CreateWindow( _T("D3D Window"), m_strWindowTitle, m_dwWindowStyle,
    ...
    if( m_hWndFocus == NULL )
        m_hWndFocus = m_hMainWnd;

    // Save window properties
    m_dwWindowStyle = GetWindowLong( m_hMainWnd, GWL_STYLE );
    GetWindowRect( m_hMainWnd, &m_rcWindowBounds );
    GetClientRect( m_hMainWnd, &m_rcWindowClient );
        ...
        case WM_SETCURSOR:
         ...
         case WM_MOUSEMOVE:
            if( m_bActive && m_bReady && m_pd3dDevice != NULL )
            {
                ...
                if( !m_bWindowed )
                    ScreenToClient( m_hMainWnd, &ptCursor );
...
HRESULT CD3DApplication::Initialize3DEnvironment()
{
	...
	//pModeInfo->dwBehavior |= D3DCREATE_PUREDEVICE|D3DCREATE_HARDWARE_VERTEXPROCESSING;
	//pModeInfo->dwBehavior |= D3DCREATE_PUREDEVICE|D3DCREATE_SOFT_VERTEXPROCESSING;
	...
	DWORD FSAAQuality = 0;
	int nLevel = D3DMULTISAMPLE_2_SAMPLES;
	...
	for( int iSampleCount = 2 ; iSampleCount <= 8 ; iSampleCount++ )
	{
		hr = m_pD3D->CheckDeviceMultiSampleType( 
			m_dwAdapter, 
			D3DDEVTYPE_HAL, 
			D3DFMT_X8R8G8B8/*displayMode.Format*/, 
			TRUE, 
	...
	if( bD3dSupport )
	{
		m_d3dpp.MultiSampleType = (D3DMULTISAMPLE_TYPE)nLevel;
		m_d3dpp.MultiSampleQuality = FSAAQuality;
	}
	else
	{
    ...
    if( SUCCEEDED(hr) )
    {
       ...
       LPDIRECT3DSURFACE9 pBackBuffer;
        ...
        if( m_bShowCursorWhenFullscreen && !m_bWindowed )
        {
            HCURSOR hCursor;
#ifdef _WIN64
            hCursor = (HCURSOR)GetClassLongPtr( m_hMainWnd, GCLP_HCURSOR );
#else
            hCursor = (HCURSOR)GetClassLong( m_hMainWnd, GCL_HCURSOR );
#endif
...
HRESULT CD3DApplication::AdjustWindowForChange()
{
    if( m_bWindowed )
    {
        ...
        SetWindowLong( m_hMainWnd, GWL_STYLE, m_dwWindowStyle );
    }
    else
    {
        // Set fullscreen-mode style
        SetWindowLong( m_hMainWnd, GWL_STYLE, WS_POPUP|WS_SYSMENU|WS_VISIBLE );
    }
    return S_OK;
}
...
HRESULT CD3DApplication::UserSelectNewDevice()
{
    ...
    if( IDOK != DialogBoxParam( (HINSTANCE)GetModuleHandle(NULL),
                                MAKEINTRESOURCE(IDD_SELECTDEVICE), m_hMainWnd,
...
INT CD3DApplication::Run()
{
    ...
    while( WM_QUIT != msg.message  )
    {
        ...
        if( bGotMsg )
        {
            ...
            if( 0 == TranslateAccelerator( m_hMainWnd, hAccel, &msg ) )
            {
...
//-----------------------------------------------------------------------------
HRESULT CD3DApplication::Render3DEnvironment(float totalTime, float deltaTime)
{
    HRESULT hr;
...
{
        //TRUETIMEBLOCK(__FUNCTION__":FrameMove");
...
{
    //TRUETIMEBLOCK(__FUNCTION__":Render()");
  
    // Render the scene as normal
    if( FAILED( hr = Render(totalTime,deltaTime) ) )
        return hr;
    ...
    {
        //TRUETIMEBLOCK(__FUNCTION__":Present()");
