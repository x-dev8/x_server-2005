...
#ifdef OneTimePad
		buf_setdofunc(self->recvbuf, OneTimePad_Decrypt_Func, NULL);
#else
...
#ifdef OneTimePad
		buf_setdofunc(self->sendbuf, OneTimePad_Encrypt_Func, NULL);
#else
...
{
	struct sockaddr_in soaddr_client;
	...
	if (self->sockfd == NET_INVALID_SOCKET)
	{
		...
		if (bind_port != 0)
		{
			memset(&soaddr_client, 0, sizeof(soaddr_client));
			soaddr_client.sin_family = AF_INET;
			soaddr_client.sin_addr.s_addr = INADDR_ANY;
			soaddr_client.sin_port = htons(bind_port);
			if (bind(self->sockfd, (struct sockaddr *)&soaddr_client, sizeof(soaddr_client)) != 0)
			{
...
void *socketer_getmsg (struct socketer *self, char *buf, size_t bufsize)
{
	...
	{
		socketer_close(self);
		if (self->flagname[0] != 0)
		{
			log_error("close socket:%s-socketer_getmsg",self->flagname);
		}
	}
	return msg;
}
...
void setflagname(struct socketer *self,const char* flagname)
{
	if (self&&flagname)
	{
		memcpy(self->flagname,flagname,4);
	}
}
	...
	for (;;)
	{
		...
		if (writebuf.len <= 0)
		{
			if (!buf_recv_end_do(self->recvbuf))
			{
				...
				if (self->flagname[0] != 0)
				{
					log_error("close socket:%s-socketer_on_recv",self->flagname);
				}
		...
		else
		{
			if (!buf_recv_end_do(self->recvbuf))
			{
				...
				if (self->flagname[0] != 0)
				{
					log_error("close socket:%s-socketer_on_recv:res:%d",self->flagname,res);
				}
				if (atom_dec(&self->ref) < 1)
				{
					log_error("%x socket recvlock:%d, sendlock:%d, fd:%d, ref:%d, thread_id:%d, connect:%d, deleted:%d", self, (int)self->recvlock, (int)self->sendlock, self->sockfd, (int)self->ref, CURRENT_THREAD, self->connected, self->deleted);
			...
			if ((!SOCKET_ERR_RW_RETRIABLE(NET_GetLastError())) || (res == 0))
			{
				...
				if (self->flagname[0] != 0)
				{
					log_error("close socket:%s-socketer_on_recv:%d,res:%d",self->flagname,WSAGetLastError(),res);
				}
				if (atom_dec(&self->ref) < 1)
				{
					log_error("%x socket recvlock:%d, sendlock:%d, fd:%d, ref:%d, thread_id:%d, connect:%d, deleted:%d", self, (int)self->recvlock, (int)self->sendlock, self->sockfd, (int)self->ref, CURRENT_THREAD, self->connected, self->deleted);
	...
	for (;;)
	{
		...
		else
		{
			if (!SOCKET_ERR_RW_RETRIABLE(NET_GetLastError()))
			{
				...
				if (self->flagname[0] != 0)
				{
					log_error("close socket:%s-socketer_on_send:%d,res:%d",self->flagname,WSAGetLastError(),res);
				}
				if (atom_dec(&self->ref) < 1)
				{
					log_error("%x socket recvlock:%d, sendlock:%d, fd:%d, ref:%d, thread_id:%d, connect:%d, deleted:%d", self, (int)self->recvlock, (int)self->sendlock, self->sockfd, (int)self->ref, CURRENT_THREAD, self->connected, self->deleted);
