...
static inline void blocklist_addwrite (struct blocklist *lst, int len)
{
	...
	remembers = lst->canusesize;
	wirsize = lst->tail->write;
	tempsize = block_getwritesize(lst->tail);
	if (tempsize != lst->canusesize)
	{
		int n =0;
		n++;
	}
	...
	//lst->canusesize = block_getwritesize(lst->tail);
...
static inline bool blocklist_pushdata (struct blocklist *lst, const char *msgbuf, int len)
{
	...
	while (writesize < len)
	{
		...
		{
			int remembers = lst->canusesize;
			int wirsize = bk->write;
			int tempsize = block_getwritesize(bk);
			if (tempsize != lst->canusesize)
			{
				int n =0;
				n++;
			...
			tempsize = block_getwritesize(bk);
			if (tempsize != lst->canusesize)
			{
				int n =0;
				n++;
