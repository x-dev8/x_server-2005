...
inline LuaObject& LuaObject::SetNil(const T& key)
{
	...
	unsigned char keyObj[sizeof(lua_TObject)];
	LuaArgNil argNil;
	detail::AssignNewTObject(m_state->m_state, (lua_TObject*)keyObj, argNil);
	detail::AssignNewTObject(m_state->m_state, (lua_TObject*)keyObj, key);
	unsigned char valueObj[sizeof(lua_TObject)];
