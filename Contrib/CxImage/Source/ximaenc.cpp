...
#endif
#if CXIMAGE_SUPPORT_DDS
	if (imagetype==CXIMAGE_SUPPORT_DDS){
		CxImageDDS newima;
		newima.Ghost(this);
		if (newima.Encode(hFile)){
			return true;
...
#endif
#if CXIMAGE_SUPPORT_DDS
		{ CxImageDDS newima; newima.CopyInfo(*this); if (newima.Decode(hFile)) { Transfer(newima); return true; } else hFile->Seek(pos,SEEK_SET); }
#endif
#if CXIMAGE_SUPPORT_ICO
...
#endif
#if CXIMAGE_SUPPORT_DDS
	if (imagetype==CXIMAGE_FORMAT_DDS){
		CxImageDDS newima;
		newima.CopyInfo(*this);
		if (newima.Decode(hFile)){
			Transfer(newima);
