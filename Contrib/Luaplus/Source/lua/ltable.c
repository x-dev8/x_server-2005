...
void luaH_removekey (lua_State *L, Table *t, Node *e) {
  Node *mp = luaH_mainposition(t, gkey(e));  /* `mp' of the node */
  (void)L;
  if (e != mp) {  /* element not in its main position? */
    while (mp->next != e) mp = mp->next;  /* find previous */
    mp->next = e->next;  /* remove `e' from its list */
	...
	if (e->next != NULL) {
      ...
      setnilvalue(gkey(e));  /* clear node `e' */
	  *mp = *e->next;
      nold->next = NULL;  /* now `mp' is free */
	  setnilvalue2n(gval(nold));
	  newvalue(gkey(nold));
      if (nold > t->firstfree)
        t->firstfree = nold;
	} else {
...
static Node *luaH_getkeyany (Table *t, const TObject *key) {
...
Node *luaH_getkeywstr (Table *t, TString *key) {
  Node *n = hashstr(t, key);
  do {  /* check whether `key' is somewhere in the chain */
...
*/
Node *luaH_getkey (Table *t, const TObject *key) {
  switch (ttype(key)) {
