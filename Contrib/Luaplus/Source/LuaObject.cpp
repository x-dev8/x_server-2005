...
void LuaObject::DeepClone(LuaObject& outObj)
{
	if (IsTable())
	{
		...
		for (LuaTableIterator it(*this); it; ++it)
		{
			...
			{
				case LuaState::TBOOLEAN:	keyObj.AssignBoolean(outObj.GetState(), it.GetKey().GetBoolean());		break;
				case LuaState::TNUMBER:		keyObj.AssignNumber(outObj.GetState(), it.GetKey().GetNumber());		break;
				case LuaState::TSTRING:		keyObj.AssignString(outObj.GetState(), it.GetKey().GetString());		break;
				case LuaState::TWSTRING:	keyObj.AssignWString(outObj.GetState(), it.GetKey().GetWString());		break;
			}

			switch (it.GetValue().Type())
			{
				case LuaState::TBOOLEAN:	outObj.SetBoolean(keyObj, it.GetValue().GetBoolean());		break;
				case LuaState::TNUMBER:		outObj.SetNumber(keyObj, it.GetValue().GetNumber());		break;
				case LuaState::TSTRING:		outObj.SetString(keyObj, it.GetValue().GetString());		break;
				case LuaState::TWSTRING:	outObj.SetWString(keyObj, it.GetValue().GetWString());		break;
				case LuaState::TTABLE:
				{
	...
	else
	{
		switch (Type())
		{
			case LuaState::TBOOLEAN:	outObj.AssignBoolean(outObj.GetState(), this->GetBoolean());		break;
			case LuaState::TNUMBER:		outObj.AssignNumber(outObj.GetState(), this->GetNumber());			break;
			case LuaState::TSTRING:		outObj.AssignString(outObj.GetState(), this->GetString());			break;
			case LuaState::TWSTRING:	outObj.AssignWString(outObj.GetState(), this->GetWString());		break;
			case LuaState::TTABLE:		DeepClone(outObj);													break;
		}
	}
}
...
LuaObject& LuaObject::RawSetTableHelper(const TObject* keyObj, const TObject* valueObj)
{
	...
	if (!ttisnil(oldval))
	{
		...
		if (ttisnil(oldval))
		{
			Node* foundKeyNode = luaH_getkey(h, keyObj);
			if (foundKeyNode)
                luaH_removekey(L, h, foundKeyNode);
		}
#endif /* LUA_REFCOUNT */
	}
