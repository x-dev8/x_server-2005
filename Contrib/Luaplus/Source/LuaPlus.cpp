...
{
    lua_State* L = lua_newthread(parentState->GetCState());
    lua_TObject tobject;
    setnilvalue2n(&tobject);
    setthvalue(&tobject, L);
    LuaObject retObj = LuaObject(parentState, &tobject);
    setnilvalue(&tobject);
...
LuaState::LuaState(bool initStandardLibrary, bool multithreaded)
{
    ...
    luaX_setnewthreadhandler( newthread_handler );
    luaX_setfreethreadhandler( freethread_handler );
