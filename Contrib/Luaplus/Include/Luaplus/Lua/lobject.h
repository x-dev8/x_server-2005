...
#define setsvalue(obj,x) \
  { TObject *i_o=(obj); lua_makeobjectbackup(i_o); \
    i_o->value.gc=cast(GCObject *, (x)); i_o->tt=LUA_TSTRING; lua_addreftobject(i_o); lua_release(L,&bak); \
    lua_assert(i_o->value.gc->gch.tt == LUA_TSTRING); }

#define setuvalue(obj,x) \
  { TObject *i_o=(obj); lua_makeobjectbackup(i_o); \
    i_o->value.gc=cast(GCObject *, (x)); i_o->tt=LUA_TUSERDATA; lua_addreftobject(i_o); lua_release(L,&bak); \
    lua_assert(i_o->value.gc->gch.tt == LUA_TUSERDATA); }

#define setthvalue(obj,x) \
  { TObject *i_o=(obj); lua_makeobjectbackup(i_o); \
    i_o->value.gc=cast(GCObject *, (x)); i_o->tt=LUA_TTHREAD; lua_addreftobject(i_o); lua_release(L,&bak); \
    lua_assert(i_o->value.gc->gch.tt == LUA_TTHREAD); }

#define setclvalue(obj,x) \
  { TObject *i_o=(obj); lua_makeobjectbackup(i_o); \
    i_o->value.gc=cast(GCObject *, (x)); i_o->tt=LUA_TFUNCTION; lua_addreftobject(i_o); lua_release(L,&bak); \
    lua_assert(i_o->value.gc->gch.tt == LUA_TFUNCTION); }

#define sethvalue(obj,x) \
  { TObject *i_o=(obj); lua_makeobjectbackup(i_o); \
    i_o->value.gc=cast(GCObject *, (x)); i_o->tt=LUA_TTABLE; lua_addreftobject(i_o); lua_release(L,&bak); \
    lua_assert(i_o->value.gc->gch.tt == LUA_TTABLE); }

#define setnilvalue(obj) { TObject *i_o=(obj); lua_release(L,i_o); i_o->tt=LUA_TNIL; i_o->value.gc = NULL; }

#define setwsvalue(obj,x) \
  { TObject *i_o=(obj); lua_makeobjectbackup(i_o); \
    i_o->value.gc=cast(GCObject *, (x)); i_o->tt=LUA_TWSTRING; lua_addreftobject(i_o); lua_release(L,&bak); \
...
#define setnilvalue2n(obj) { TObject *i_o=(obj); i_o->tt=LUA_TNIL; i_o->value.gc = NULL; }
