...
std::string u_user="cd";
std::string u_pwd="123";
std::string u_url="127.0.0.1";
std::string u_apppara="c";
bool bUploadDump=false;

#pragma warning( push )
#pragma warning( disable : 4311 )
void MiniDumper::set_upload_info(std::string user_,std::string pwd_,std::string ip_,bool bUpload)
{
	u_url=ip_;
	u_user=user_;
	u_pwd=pwd_;
...
void MiniDumper::set_upload_info(std::string apppara)
{
	u_apppara=apppara;
}
void MiniDumper::set_upload_enable(bool bEnable)
{
	bUploadDump=bEnable;
}
...
{
	if(!m_szAppName)return EXCEPTION_EXECUTE_HANDLER;//
    ...
    */
    ...
    if(GetCurrentDirectory(sizeof(szDbgHelpPath)/sizeof(char) - 1,szDbgHelpPath) != 0)
    {
        strFilePath = szDbgHelpPath;
        strFilePath += "\\dbghelp.dll";
        hDll = LoadLibrary(strFilePath.c_str());
    }
...
// #endif // GAME_CLIENT
					...
					if(bUploadDump)
					{
						...
						sprintf(buf,"CrashRptUpload.exe %s %s %s %s %s %s",u_user.c_str(),u_pwd.c_str(),u_url.c_str(),szDumpPath,szCurName,u_apppara.c_str());
						//	CreateProcess("CrashRptUpload.exe ", buf, NULL, NULL, FALSE, 0, NULL, NULL, &si, &pi);
						CreateProcess(NULL, (LPSTR)buf, NULL, NULL, FALSE, 0, NULL, NULL, &si, &pi);
					}
				//	if(bUploadDump)
