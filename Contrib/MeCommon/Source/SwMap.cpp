...
bool SwMap::Create( const SwMap& templateMap )
{
	if ( NULL == templateMap.m_pkMapData )
		return false;
	...
	m_pkPathFinder = new BinaryAStarPF( templateMap.m_pkMapData, templateMap.m_nWidth, templateMap.m_nHeight );
	m_nWidth = templateMap.m_nWidth;
	m_nHeight = templateMap.m_nHeight;
	m_fRate = templateMap.m_fRate;
...
int SwMap::CalPFWeight( float startx,float starty,float endx,float endy ) 
{
	...
	{
		float ret = 0;
		for (size_t i = 1; i < vecBestPath.size(); ++i)
		{
...
void SwMap::DelBlockPoint( float x,float y )
{
	if( !m_pkPathFinder )
		return ;
	int posx = GameCoordToMap(x);
...
{
	unsigned long dwWidthByteCount = 0;
// 	BOOL bReadMapSuccess = FALSE;
	...
	if( stream != NULL)
	{
		packet_namespace::w_fread(&mapHeader,sizeof( TileDataMapHeader ),1,stream);
		...
		m_pkMapData = new unsigned char[ mapHeader.width * mapHeader.height];
		if (mapHeader.version < 3)
		{
			unsigned long  dwWidthByteCount = ( mapHeader.width + 7) >> 3;
			unsigned char* retTempData = new unsigned char[ dwWidthByteCount * mapHeader.height];
			packet_namespace::w_fread(retTempData,sizeof(unsigned char)*dwWidthByteCount * mapHeader.height,1,stream);

			for (unsigned long i = 0; i <  mapHeader.height; i++)
			{
				for (unsigned long j = 0; j < mapHeader.width; j++)
				{
		...
		{
			unsigned long  dwWidthByteCount = mapHeader.width;;
			packet_namespace::w_fread(m_pkMapData,sizeof(unsigned char)*dwWidthByteCount * mapHeader.height,1,stream);
		}
